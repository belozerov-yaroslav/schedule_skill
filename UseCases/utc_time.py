...
def get_utc_time(time: datetime, time_zone):  # получить из местного времени время на utc
    delta = timedelta(hours=get_utc_diff(time_zone))
    new_time = time - delta
    return new_time


def get_zone_time(time: datetime, time_zone):  # получить из utc местное время
    delta = timedelta(hours=get_utc_diff(time_zone))
    new_time = time + delta
    return new_time
...
def get_utc_diff(time_zone):
    # получение отличия часового пояса от UTC
    inf = {}
    ...
    except KeyError:  # если нет часового пояса в файле
        raise WrongTimezone
